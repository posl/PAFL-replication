...
class AALERGIA():
        ...
        while len(queue) > 0:
            ...
            for sigma in self.alphabet:
                ...
                if next_node in self.prefix2freq:
                    node_id += 1
                    prefix2id[next_node] = node_id # ノード=>ノードID
                    id2prefix[node_id] = next_node # ノードID=>ノード
                    id2parent[node_id] = prefix2id[c_node] # ノードID=>親ノードのID
                    id2children[prefix2id[c_node]].append(node_id) # ノードID=>子ノードのIDの配列
                    id2actions[prefix2id[c_node]].append(sigma) # ノードID=>受け付けるシンボル（抽象状態）の配列
    ...
    def makeDLMC(self):
        ...
        for id in self.id2children:
            ...
            for action, child in zip(actions, children):
                # trans_func : ノードのidとそのノードで受け付けるシンボル（抽象状態）=>子ノードIDの対応表（これが遷移行列）
        ...
        for id in self.id2children:
            ...
            for action, child in zip(actions, children):
                ...
                # trans_wfunc：ノードのidとそのノードで受け付けるシンボル（抽象状態）=>子ノードIDの出現回数の対応表（これが重み付き遷移行列）
    ...
    def output_prism(self, dffa, model_name):
        ...
        for id in old_ids:
            ...
            new_id = id2newId[id]
            label = self.last_symbol(prefix) # trace の最後の文字をラベルとする
