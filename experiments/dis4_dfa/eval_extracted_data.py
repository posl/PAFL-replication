    ...
    for n in range(start_n, 6):
      for k in range(start_k, 12, 2):
        for theta in [10, 50]:
          ...
          indices_path = os.path.join(save_dir, f'names_indices_theta={theta}.npz')
          names_indices = np.load(indices_path, allow_pickle=True)
          names, indices_list = names_indices['names'], names_indices['indices']
          for m_idx, method_name in enumerate(names):
            ...
            save_pickle(os.path.join(save_dir, f"{method_name}_pred_result_theta={theta}.pkl"), result)
