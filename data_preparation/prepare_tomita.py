...
import random
import numpy as np
from data_preparation import tomita_fa
...
class Generator(object):
  ...
  def __init__(self, grammarType, len_pool, max_iteries, max_size_per_len, ub_neg_num=20):
    ...
    if self.grammarType == DataSet.Tomita1:
      self.dfa = tomita_fa.Tomita_1()

    if self.grammarType == DataSet.Tomita2:
      self.dfa = tomita_fa.Tomita_2()

    if self.grammarType == DataSet.Tomita3:
      self.dfa = tomita_fa.Tomita_3()

    if self.grammarType == DataSet.Tomita4:
      self.dfa = tomita_fa.Tomita_4()

    if self.grammarType == DataSet.Tomita5:
      self.dfa = tomita_fa.Tomita_5()

    if self.grammarType == DataSet.Tomita6:
      self.dfa = tomita_fa.Tomita_6()

    if self.grammarType == DataSet.Tomita7:
...
def divide_tomita(tomita_x):
  ...
  #####################################################
  if tomita_x == DataSet.Tomita2  and sum(test_y) == 0:
  ...
  save_path = get_path(DataPath.TOMITA.PROCESSED_DATA).format(tomita_x[-1])
  save_wv_matrix_path = get_path(DataPath.TOMITA.WV_MATRIX).format(tomita_x[-1])
  print("train: {}({}), test:{}({})".format(len(data["train_y"]), sum(data["train_y"]), len(data["test_y"]), sum(data["test_y"])))
